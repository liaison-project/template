...
#if defined(NDEBUG)
# error "Xxxcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x7f970993675723dc8b212aff07a0dd005913207e6fcf4bcb697f6d0c8f6b130b");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Xxxcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Xxxcoin Signed Message:\n";
...
{
    // Xxxcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Xxxcoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Xxxcoin: 1 days
static const int64 nTargetSpacing = 2 * 60; // Xxxcoin: 2 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Xxxcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Xxxcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x7f970993675723dc8b212aff07a0dd005913207e6fcf4bcb697f6d0c8f6b130b");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "NY Times 24/Oct/2017 Do you think that is air you are breathing ";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1508870784;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 1188278;
        ...
        assert(block.hashMerkleRoot == uint256("0x3aa205cb9962d60ef183d69eb3361b41f1716aec0825b422003649a10e9e7ade"));
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Xxxcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("XxxcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("XxxcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static XxxcoinMiner(CWallet *pwallet)
{
    printf("XxxcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("xxxcoin-miner");
    ...
    try { loop {
        ...
        printf("Running XxxcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("XxxcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&XxxcoinMiner, pwallet));
}
